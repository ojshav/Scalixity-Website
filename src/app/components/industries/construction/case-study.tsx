...
export function CaseStudy() {
  return (
    <section className="bg-gradient-to-r from-[#FF6B6B] via-[#F94C10] to-[#FF2E63] py-24">
      <div className="container mx-auto px-6">
        <h2 className="text-5xl font-extrabold text-white text-center mb-16 tracking-wide">AI-Powered Construction Innovations</h2>
        {caseStudies.map((study, index) => (
          <div key={index} className="bg-white bg-opacity-20 rounded-2xl overflow-hidden border border-white border-opacity-30 mb-16 shadow-lg hover:shadow-2xl transition-all">
            <div className="grid md:grid-cols-2 gap-10 items-center">
              <div className="p-12">
                <h3 className="text-3xl font-bold text-white mb-6">{study.title}</h3>
                <p className="text-lg text-gray-300 mb-8 leading-relaxed">{study.description}</p>
                <ul className="space-y-3">
                  {study.highlights.map((highlight, idx) => (
                    <li key={idx} className="flex items-start">
                      <span className="text-yellow-400 text-xl mr-3">⚡</span>
                      <span className="text-gray-100 text-lg">{highlight}</span>
                    </li>
                  ))}
                </ul>
              </div>
              <div className="relative h-full min-h-[350px]">
                <Image src={study.image} alt={study.title} layout="fill" objectFit="cover" className="rounded-r-2xl"/>
              </div>
            </div>
          </div>
        ))}
