            ...
            {processSteps.map((step, index) => (
              ...
              >
                <div className="absolute left-0 top-2 w-[30px] h-[30px] bg-white rounded-full flex items-center justify-center">
                  <div className="w-2 h-2 bg-[#5B0BB5] rounded-full" />
                </div>

                <div className="bg-white p-8 rounded-xl border border-white/20">
                  <h3 className="text-xl font-bold text-[#5B0BB5] mb-4">{step.title}</h3>
                  <p className="text-[#5B0BB5]/80 leading-relaxed">{step.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
