    ...
    <section className="bg-[#A8B2E7] py-24"> {/* Lavender background for the section */}
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <span className="text-sm text-black uppercase tracking-wider">WHAT WE OFFER</span> {/* Black text */}
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {services.map((service, index) => (
            <motion.div
              ...
              className="bg-[#F3F1EB] p-8 rounded-xl border border-black hover:border-primary/50 transition-colors" /* Soft, warm beige box */
            >
              <h3 className="text-xl font-bold text-black mb-4">{service.title}</h3> {/* Black text */}
              <p className="text-black/80 leading-relaxed">{service.description}</p> {/* Black text */}
            </motion.div>
          ))}
        </div>
      </div>
