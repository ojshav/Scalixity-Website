      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <Link href={blogPosts[0].link} className="block group col-span-2 md:col-span-1">
            <div className="bg-[#590178] border border-black rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300">
              <div className="relative h-96 w-full">
                <Image src={blogPosts[0].image} alt={blogPosts[0].title} layout="fill" className="object-cover w-full h-full" />
              </div>
              <div className="p-8">
                <span className="text-white text-sm mb-2 block">{blogPosts[0].category}</span>
                <h3 className="text-3xl font-bold text-white mb-4 ">
                  {blogPosts[0].title}
                </h3>
                <p className="text-white">{blogPosts[0].excerpt}</p>
                <span className="text-white text-sm mt-4 block">{blogPosts[0].date}</span>
              </div>
            </div>
          </Link>
          ...
          <div className="grid gap-8">
            {blogPosts.slice(1).map((post, index) => (
              <Link key={index} href={post.link} className="block group">
                <div className="bg-[#590178] border border-black rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300">
